'''
Run
cargo r --features csv --features serde_json --features counts_by_round --bin mal_example --release
python data/mal.py
'''
...
anime_names = {
    row['anime_id']: row['title']
    for _, row in anime_df.iterrows()
}

with open('./out/mal/mal.json', 'r') as f:
    j = json.load(f)
...
# df.index = df.index.map(lambda x: x[:20])
...
plt.tight_layout()
plt.savefig('out/mal/rounds.png')
plt.close()
...
plt.legend()
plt.tight_layout()
plt.savefig('out/mal/stacked.png')
plt.close()
