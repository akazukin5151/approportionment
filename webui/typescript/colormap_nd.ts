...
export function transform_to_radial(
	...
	const seat_coords =
		calculate_seat_coords(all_seats_by_party, party_coords, should_expand_points, ncols)
...
function calculate_seat_coords(
	...
	return normalize(all_seats_by_party).map(row => {
		...
		if (should_expand_points) {
			return expand_points(party_coords, { grid_x, grid_y }, ncols)
		} else {
