...
export function add_drop_listeners(
  ...
  div.addEventListener('drop', ev => on_drop(ev, simulation_canvas))
  div.addEventListener('dragover', on_drag_over)
}

function on_drop(ev: DragEvent, simulation_canvas: Canvas): void {
  ev.preventDefault()
...
function on_drag_over(ev: DragEvent): void {
  ev.preventDefault()
